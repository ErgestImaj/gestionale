...
<script>
	export default {
		...
		methods: {
			getStructure(){
				axios.get(`/amministrazione/api/struture/${getUrlData(3)}/edit/`)
					.then(response => {
						this.showParentStructure(response.data.type);
           this.strutura = response.data
           this.strutura.nome = response.data.name
           this.strutura.parent= response.data.parent_structure_id
						this.strutura.accredit = this.getAccreditamneto(response.data.status);
					})
					.catch(error => {
					});
			},
			...
			getAccreditamneto(value){
				...
				if (value.status_miur == 1){
					accredit.push(1)
					this.strutura.accredit_miur = value.date_miur
					this.miur = true
				}
				if (value.status == 1){
					accredit.push(2)
				  this.strutura.accredit_mf =value.date
					this.mf = true
				}
				if (value.status_iiq == 1){
					accredit.push(3)
					this.strutura.accredit_iiq = value.date_iiq
					this.iiq = true
				}
				if (value.status_lrn == 1){
					accredit.push(4)
					this.strutura.accredit_lrn = value.date_lrn
					this.lrn = true
				}
				if (value.status_dile == 1){
					accredit.push(5)
					this.strutura.accredit_dile = value.date_dile
					this.dile = true
				}

				return accredit;
			},
			showParentStructure(type) {
				if (type == 1) {
					return false;
				}
				axios.get(`/amministrazione/api/struture/${type}/parent`)
					.then(response => {
			...
			update() {
				if (!this.submiting) {
					...
					axios.post(`/amministrazione/api/structure/${this.strutura.hashid}/update`,
						formData,
						{
							headers: {
