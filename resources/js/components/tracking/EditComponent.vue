							...
							<v-row>
								<v-col cols="12" sm="12" class="gel">
									...
									<v-btn-toggle v-model="content.status" group mandatory color="primary">
										...
										<v-btn :value="4">Non Ricevuto</v-btn>
    ...
    export default {
        ...
        methods: {
            update() {
							if (!this.submiting){
								this.submiting = true;
								axios.post(`/tracking/${this.content.hashid}/update`,this.content)
									.then(response => {
                ...
                axios.get(`/tracking/${getUrlData(2)}`).then(response => {
            ...
            receivedDate(val) {
                ...
                this.exams.forEach(e => {
                    if (e.id == val) {
                        this.$set(this.content, 'structur', e.owner.firstname.toString());
