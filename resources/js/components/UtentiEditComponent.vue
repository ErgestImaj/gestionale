							...
							<v-row>
								<v-col cols="12" sm="12" class="gel">
									...
									>
										...
										<v-col cols="2" v-if="user.cv" class="text-right">
											<v-btn class="icon-space" target="_blank" :href="user.content_path+user.token+'/'+user.cv" color="primary" dark>
												<v-icon dark>mdi-eye</v-icon>
											</v-btn>
										</v-col>
									</v-row>
									...
									<v-row>
										...
										<v-col cols="2" v-if="user.document" class="text-right">
											<v-btn class="icon-space" target="_blank" :href="user.content_path+user.token+'/'+user.document" color="primary" dark>
												<v-icon dark>mdi-eye</v-icon>
											</v-btn>
										</v-col>
									</v-row>
		...
		methods: {
			...
			update() {
				if (!this.submiting) {
					...
					axios.post(
						`/utenti/api/${getUrlData(2)}/updateadvanceduser`,
						formData,
						{
							headers: {
			...
			getUser(){
				axios.get(`/utenti/api/${getUrlData(2)}/editadvanceduser`)
					.then(response => {
						if (response.data) {
